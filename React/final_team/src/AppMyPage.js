...
const ViewUserData = ({loginID, password, name, address, phone, email, gender, year, month, day}) => {
  return <div id="MyInfo">

    <div>개인정보</div>
    <ul>
      <li id="myId">     <span>아이디 : </span>   <span>{loginID}</span></li>
      <li id="myPw">     <span>비밀번호 : </span> <span>{password}</span></li>
      <li id="myName">   <span>이름 : </span>     <span>{name}</span></li>
      <li id="myAdress"> <span>주소 : </span>     <span>{address}</span></li>
